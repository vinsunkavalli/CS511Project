...
def update_graph(cell: str, cell_runtime: float, start_time: float, input_variables: set,
    ...
    output_vss_create = set(graph.create_variable_snapshot(k, False) for k in created_and_modified_variables)
    output_vss_delete = set(graph.create_variable_snapshot(k, True) for k in deleted_variables)
